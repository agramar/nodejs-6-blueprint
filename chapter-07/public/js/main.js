(function () {
    ...
    socket.on('new message', function (data) {
        ...
        data.msg ? ($chat.append('<li>[${currentHours}:${currentMinutes}]<strong> ${data.user}:</strong>${data.msg}</li>')) : alert('Blank message not allow!');
    });
    ...
    $userForm.submit(function (e) {
        ...
        socket.emit('new user', $username.val(), function (data) {
            data ? ($userForm.hide(), $messageArea.show()) : alert("Ohps. What's your name!");
        });
        $username.val('');
    });
    ...
    socket.on('get userList', function (data) {
        ...
        $onlineUsersHeader.html(
            '<span class="card-title">Users in the room: </span>' +
            '<span class="label labelsuccess">${data.length}</span>'
        );
    });
})();
